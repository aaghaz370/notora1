...
async function loadCategory(category) {
  try {
    ...
    books.slice(0, 30).forEach(book => {
      ...
      // ===== When card is clicked -> open book.html =====
...
// ===== Add to Bookmark (Smooth Animation) =====
function addToBookmarks(book) {
  ...
  if (!exists) {
    ...
    // Small animation (no alert)
    const note = document.createElement("div");
    note.classList.add("bookmark-added");
    note.textContent = "⭐ Added to Bookmarks!";
